...
process.env.EAT_TAG_SECRET = 'test-tag-secret-not-a-real-one';
...
(async function () {
  ...
  delete process.env.EAT_REQUIRE_TAG_SIG;
  ...
  process.env.EAT_REQUIRE_TAG_SIG = '1';
  ...
  await L.clearLimit('eatminot:rated:' + d3 + ':' + VENUE);
  r = await call(rate, { id: VENUE, t: sig, deviceId: d3, stars: 5, upvote: true });
  ...
  ok(/^EAT[A-Z2-9]{10}$/.test(code), 'the code is server-minted and long enough not to be enumerable: ' + code);
  ...
  r = await call(coupon, { action: 'redeem', code: 'EATZZZZZZZZZZ', pin: '135790' });
  ...
  await L.clearLimit('eatminot:rated:' + d4 + ':' + VENUE);
  ...
  await L.kvSet('eatminot:coupon:' + expired.code, JSON.stringify(expired));
