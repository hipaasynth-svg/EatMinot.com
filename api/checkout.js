...
// Standard tier: $59/mo, no trial, billed immediately.
// Founding Three: exactly 3 venues total (admin-granted via foundingOffer, see admin.js)
// get a 10-week free trial, then $79/mo — see api/_lib.js FOUNDING_* constants. A venue
// that already redeemed the offer (r.founding) keeps the $79 rate on any future checkout
// (e.g. resubscribing after a cancellation) but does not get a second free trial.
module.exports = async function (req, res) {
  ...
  try {
    ...
    var base = req.headers.origin || ('https://' + (req.headers.host || 'eatminot.com'));
    var params = {
      mode: 'subscription',
      ...
      'line_items[0][price_data][unit_amount]': isFounding ? String(L.FOUNDING_PRICE_CENTS) : '5900',
      'line_items[0][price_data][recurring][interval]': 'month',
      'line_items[0][price_data][product_data][name]': (isFounding ? 'EatMinot Founding — ' : 'EatMinot Claimed — ') + r.name
    };
    if (isNewFounding) {
      // Only the first checkout that actually redeems the offer gets the free trial —
      // a later resubscription (r.founding already true) keeps the $79 rate, no second trial.
