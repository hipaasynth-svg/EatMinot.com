...
module.exports = async function (req, res) {
  ...
  try {
    ...
    // First-run claim: an unclaimed listing is claimed by presenting that venue's own
    // claim code — a random per-venue secret shown only in the admin console and handed
    // over in person with the tags (?owner=<id>&c=<code>). The code is required because
    // venue ids are sequential and printed on every tag, so the id alone proves nothing;
    // without it, anyone could have seized any listing they hadn't claimed yet.
    // One-time by construction — the claimed flag closes this path afterwards.
    if (b.action === 'claim') {
    ...
    if (b.action === 'update') {
      ...
      await L.updateProfile(profile.id, function (r) {
        ...
        // member's own phone. Hashed like the password — it is never readable back, so
        // an owner who forgets it sets a new one. '' clears it, which switches
        // redemption off for this venue rather than leaving a guessable default.
