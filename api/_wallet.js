...
// balance and a QR that reopens https://eatminot.com/?r=<venue>&dev=<token>, so the
// pass itself re-links a wiped phone to its backup. The device token is anonymous.
var crypto = require('crypto');

var SITE = 'https://eatminot.com';
var CLASS_SUFFIX = 'eatminot_loyalty_v1';
...
function classBody() {
  return {
    id: classId(),
    issuerName: 'EatMinot',
    programName: 'EatMinot Punch Card',
...
function objectBody(token, venueId, venueName, done, total) {
  return {
    ...
    accountName: venueName || 'EatMinot member',
    loyaltyPoints: { label: 'Punches', balance: { string: (done || 0) + ' / ' + (total || DEFAULT_TOTAL) } },
    textModulesData: [{ header: venueName || 'EatMinot', body: 'Tap in-store to earn punches toward your reward.' }],
...
var OFFER_CLASS_SUFFIX = 'eatminot_reward_v1';
...
function offerObjectBody(token, code, venueName, reward, expiresAt) {
  var o = {
    ...
    textModulesData: [
      { header: venueName || 'EatMinot', body: reward || 'Reward earned!' },
...
/* ============================ Apple Wallet (.pkpass) ============================
   A real, signed store-card pass — env-gated exactly like Google. Required vars:
     APPLE_PASS_TYPE_ID          e.g. pass.com.eatminot.loyalty
...
var APPLE_ORG = 'EatMinot', APPLE_DESC = 'EatMinot punch card';
var PASS_BG = 'rgb(28,25,23)', PASS_FG = 'rgb(248,245,239)', PASS_LABEL = 'rgb(213,87,43)';
