...
//
//   1. The tag signature (?t=), an HMAC of the venue id under EAT_TAG_SECRET. Tags
//      already in venues carry no signature, so this is only REFUSED when
//      EAT_REQUIRE_TAG_SIG=1 — reprint or reprogram from the admin console, then set
...
module.exports = async function (req, res) {
  ...
  try {
    ...
    var ipLim = await L.rateLimit('eatminot:rl:ip:' + ip, L.RATE_IP_MAX, L.RATE_IP_WINDOW);
    ...
    var won = await L.claimOnce('eatminot:rated:' + dev + ':' + profile.id, L.RATE_ONCE_TTL);
