...
module.exports = async function (req, res) {
  try {
    ...
    var total = Math.max(1, Math.min(1000, parseInt(b.total, 10) || 5));
...
async function serveApple(q, res) {
  ...
  var total = Math.max(1, Math.min(1000, parseInt(q.total, 10) || 5));
  var buf = W.applePkpass(dev, venueId, profile.name, done, total);
  if (!buf) { L.json(res, 502, { error: 'wallet_failed' }); return; }
  res.statusCode = 200;
  res.setHeader('Content-Type', 'application/vnd.apple.pkpass');
  res.setHeader('Content-Disposition', 'attachment; filename="eatminot.pkpass"');
